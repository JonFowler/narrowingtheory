...
data Exp (X : VarSet) {n : ℕ} (Γ : Cxt n) : (t : Type) → Set where
   ...
   fvar : ∀{n' t}{Γ' : Cxt n'} → Subst X Γ' Γ → (x : X Γ' t) → 
              Exp X Γ t
...
sucExp Γ (fvar sub x) = fvar (sucE Γ sub) x 

replaceVar :  ∀{s s' u t}{X : VarSet}{Γ' : Cxt s'} → (Γ : Cxt s) → 
          Var (Γ ++ u ∷ Γ') t → Exp X Γ' u → Var (Γ ++ Γ') t ⊎ Exp X (Γ ++ Γ') t
replaceVar [] (inj₁ refl) e' = inj₂ e'
replaceVar [] (inj₂ y) e' = inj₁ y
replaceVar (x ∷ Γ) (inj₁ eq) e' = inj₁ (inj₁ eq)
replaceVar (x ∷ Γ) (inj₂ y) e' with replaceVar Γ y e'
replaceVar (x ∷ Γ) (inj₂ y) e' | inj₁ y' = inj₁ (inj₂ y')
replaceVar (x ∷ Γ) (inj₂ y) e' | inj₂ e = inj₂ (sucExp [] e)
...
replace Γ (case {u}{v} e e₁ e₂) e' = case (replace Γ e e') 
                                   (replace (u ∷ Γ) e₁ e') 
                                   (replace (v ∷ Γ) e₂ e')
replace Γ (var v) e' with replaceVar Γ v e'
replace Γ (var v) e' | inj₁ v' = var v'
replace Γ (var v) e' | inj₂ e = e
replace Γ (fvar s x) e' = fvar (repl Γ e' s) x
...
var v >>=ₛ σ = var v
fvar [] x >>=ₛ σ = σ x
fvar (sucE Γ s) x >>=ₛ σ = sucExp Γ (fvar s x >>=ₛ σ)
fvar (repl Γ e s) x >>=ₛ σ = replace Γ (fvar s x >>=ₛ σ) (e >>=ₛ σ)
...
lift-sucExp Γ σ (inL e) = cong inL {!!} -- ((lift-sucExp Γ σ ?))
lift-sucExp Γ σ (inR e) = cong inR {!!} -- ((lift-sucExp Γ σ ?))
